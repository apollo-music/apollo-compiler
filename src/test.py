...
if __name__ == '__main__':
	print("\n----------------------- Testing: Lexer -------------------------------")
	testlexer.run()
	for test in [test for test in tests_files if test['type'] == 'TEST_ALL' or test['type'] == 'TEST_LEX']:
		print(test)

	print("\n----------------------- Testing: Parser ------------------------------")
	testparser.run()
	for test in [test for test in tests_files if test['type'] == 'TEST_ALL' or test['type'] == 'TEST_PAR']:
		print(test)

	print("\n----------------------- Testing: Semantic Analisis -------------------")
	testsem.run()
	for test in [test for test in tests_files if test['type'] == 'TEST_ALL' or test['type'] == 'SEMAN_AN']:
		print(test)
	
	print("\n----------------------- Testing: CodeGen -----------------------------")
	testcodegen.run()
	for test in [test for test in tests_files if test['type'] == 'TEST_ALL' or test['type'] == 'CODE_GEN']:
		print(test)
