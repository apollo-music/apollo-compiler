...
def p_param_AMP(p):
	...
	p[0] = AST.AmpNode(AST.ExpressionNode([p[3], AST.EmptyNode()]))

def p_param_DUR(p):
	'param : DUR EQUAL exp2'
	p[0] = AST.DurNode(AST.ExpressionNode([p[3], AST.EmptyNode()]))

def p_param_INSTR(p):
	'param : INSTR EQUAL exp2'
	p[0] = AST.InstrNode(AST.ExpressionNode([p[3], AST.EmptyNode()]))

def p_param_TONE(p):
	'param : TONE EQUAL exp2'
	p[0] = AST.ToneNode(AST.ExpressionNode([p[3], AST.EmptyNode()]))

def p_param_SLEEP(p):
	'param : SLEEP EQUAL exp2'
	p[0] = AST.SleepNode(AST.ExpressionNode([p[3], AST.EmptyNode()]))
